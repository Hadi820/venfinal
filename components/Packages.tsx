...
interface PackagesProps {
    packages: Package[];
    packageCrud: CrudProps<Package>;
    addOns: AddOn[];
    addOnCrud: CrudProps<AddOn>;
    projects: Project[];
}

const Packages: React.FC<PackagesProps> = ({ packages, packageCrud, addOns, addOnCrud, projects }) => {
  ...
  const handlePackageSubmit = async (e: React.FormEvent) => {
    ...
    if (packageEditMode) {
        await packageCrud.update(packageEditMode, packageData);
    } else {
        await packageCrud.add(packageData);
    }
  ...
  const handleAddOnSubmit = async (e: React.FormEvent) => {
    ...
    if (addOnEditMode) {
        await addOnCrud.update(addOnEditMode, addOnData);
    } else {
        await addOnCrud.add(addOnData);
    }
