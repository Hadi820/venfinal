...
const QuickLeadForm: React.FC<{
    ...
    userProfile: Profile;
    showNotification: (message: string) => void;
}> = ({ leadCrud, userProfile, showNotification }) => {
...
const Leads: React.FC<LeadsProps> = ({
    leads: leadsProp, leadCrud, clients, clientCrud, projects, projectCrud, packages, addOns, transactions, transactionCrud, userProfile, showNotification, cards, cardCrud, pockets, pocketCrud, promoCodes, promoCodeCrud
}) => {
    ...
    const leadStats = useMemo(() => {
        ...
        const topSource = Object.keys(sourceCounts).length > 0
            ? Object.entries(sourceCounts).sort(([,a],[,b]) => (b as number)-(a as number))[0][0]
            : 'N/A';
    ...
    const sortedLocations = Object.entries(locationCounts).sort(([, a], [, b]) => (b as number) - (a as number));
    ...
    const bookingFormUrl = useMemo(() => `${window.location.origin}${window.location.pathname}#/public-booking`, []);
    const prospectFormUrl = useMemo(() => `${window.location.origin}${window.location.pathname}#/public-lead-form`, []);
    ...
    const handleDrop = async (e: React.DragEvent<HTMLDivElement>, newStatus: LeadStatus) => {
        ...
        if (lead && lead.status !== newStatus) {
            if (newStatus === LeadStatus.CONVERTED) {
                ...
                await leadCrud.update(leadId, { status: newStatus });
            }
        }
    ...
    const handleConvertLead = async (e: React.FormEvent) => {
        ...
        const newProjectData = {
            projectName: conversionForm.projectName, clientName: newClient.name, clientId: newClient.id,
            ...
            status: 'Dikonfirmasi' as const, totalCost: totalProject, amountPaid: conversionForm.dp,
        ...
        await leadCrud.update(leadToConvert.id, { status: LeadStatus.CONVERTED });
        ...
        if (newProject && newProject.amountPaid > 0) {
            const newTransactionData = {
                ...
                amount: newProject.amountPaid, type: TransactionType.INCOME, projectId: newProject.id, category: 'DP Proyek' as const,
                method: 'Transfer Bank' as const, cardId: conversionForm.dpDestinationCardId,
            };
            await transactionCrud.add(newTransactionData);
            const cardToUpdate = cards.find(c => c.id === conversionForm.dpDestinationCardId);
            if(cardToUpdate) {
    ...
    const handleUpdateLead = async () => {
        ...
        const updatedLead = await leadCrud.update(selectedLead.id, { notes: editedLeadNotes, whatsapp: editedLeadWhatsapp });
        if (updatedLead) {
    ...
    return (
        <div className="space-y-6">
            ...
            <QuickLeadForm leadCrud={leadCrud} userProfile={userProfile} showNotification={showNotification} />
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center pt-6 border-t border-brand-border">
                                <div>
                                    ...
                                    >
                                        <SendIcon className="w-4 h-4" /> Bagikan Halaman Paket
                                    </button>
                                )}
                                {selectedLead.status === LeadStatus.FOLLOW_UP && (
                ...
                <div className="max-h-[60vh] overflow-y-auto pr-2">
                    {activeStatModal === 'source' ? (
                        <div className="space-y-3">
                            {Object.entries(leadStats.sourceCounts).sort(([,a],[,b]) => (b as number)-(a as number)).map(([source, count]) => (
