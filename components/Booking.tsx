...
const Booking: React.FC<BookingProps> = ({ leads, clients, projects, packages, handleNavigation, showNotification }) => {
    ...
    const [activeStatModal, setActiveStatModal] = useState<'total' | 'value' | 'dp' | null>(null);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div onClick={() => setActiveStatModal('dp')} className="cursor-pointer transition-transform duration-200 hover:scale-105">
                                ...
                                <tr key={booking.lead.id} className="hover:bg-brand-bg">
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        {booking.project.dpProofUrl ? (
                                            <button onClick={() => setViewingProofUrl(booking.project.dpProofUrl)} className="text-sm font-semibold text-blue-400 hover:underline inline-flex items-center gap-1.5">
            ...
            <Modal isOpen={!!viewingProofUrl} onClose={() => setViewingProofUrl(null)} title="Bukti Pembayaran">
             ...
             <Modal isOpen={!!activeStatModal} onClose={() => setActiveStatModal(null)} title={
                activeStatModal === 'total' ? 'Rincian Total Booking' :
                activeStatModal === 'value' ? 'Rincian Nilai Booking' :
                'Rincian DP Terkumpul'
            }>
                <div className="max-h-[60vh] overflow-y-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-brand-bg">
                            <tr>
                                <th className="p-2 text-left font-medium text-brand-text-secondary">Nama Proyek</th>
                                <th className="p-2 text-left font-medium text-brand-text-secondary">Klien</th>
                                {activeStatModal === 'value' && <th className="p-2 text-right font-medium text-brand-text-secondary">Total Nilai</th>}
                                {activeStatModal === 'dp' && <th className="p-2 text-right font-medium text-brand-text-secondary">DP Dibayar</th>}
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-brand-border">
                            {filteredBookings.map(booking => (
                                <tr key={booking.project.id}>
                                    <td className="p-2 font-semibold">{booking.project.projectName}</td>
                                    <td className="p-2">{booking.client.name}</td>
                                    {activeStatModal === 'value' && <td className="p-2 text-right">{formatCurrency(booking.project.totalCost)}</td>}
                                    {activeStatModal === 'dp' && <td className="p-2 text-right">{formatCurrency(booking.project.amountPaid)}</td>}
                                </tr>
                            ))}
                        </tbody>
                    </table>
