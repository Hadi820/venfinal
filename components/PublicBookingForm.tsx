...
const PublicBookingForm: React.FC<PublicBookingFormProps> = ({ 
    clientCrud, clients, projectCrud, packages, addOns, transactionCrud, userProfile, cards, cardCrud, pockets, pocketCrud, promoCodes, promoCodeCrud, showNotification, leadCrud
}) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (discountAmount > 0 && formData.promoCode) {
            promoCodeApplied = promoCodes.find(p => p.code === formData.promoCode.toUpperCase().trim());
        }
        ...
        let newClient: Client | undefined = existingClient;
        if (!newClient) {
            const newClientData = {
            ...
            newClient = await clientCrud.add(newClientData);
            if (!newClient) {
                showNotification('Gagal membuat klien baru.');
                setIsSubmitting(false);
                return;
            }
        } else {
            // Optionally update lastContact
            await clientCrud.update(newClient.id, { lastContact: new Date().toISOString(), instagram: formData.instagram || newClient.instagram });
        }
        ...
        const newLeadData = {
            ...
            date: new Date().toISOString().split('T')[0],
            notes: `Dikonversi secara otomatis dari formulir pemesanan publik. Proyek: ${newProjectData.projectName}. Klien ID: ${newClient.id}`
        };
        await leadCrud.add(newLeadData);

        if (promoCodeApplied) {
            await promoCodeCrud.update(promoCodeApplied.id, { usageCount: promoCodeApplied.usageCount + 1 });
        }

        if (newProject && dpAmount > 0) {
            const newTransactionData = {
                date: new Date().toISOString().split('T')[0], description: `DP Proyek ${newProject.projectName}`,
                amount: dpAmount, type: TransactionType.INCOME, projectId: newProject.id, category: 'DP Proyek' as const,
                method: 'Transfer Bank' as const, cardId: destinationCard.id,
            };
            await transactionCrud.add(newTransactionData);
            await cardCrud.update(destinationCard.id, { balance: destinationCard.balance + dpAmount });
        }
        ...
        return (
            <div className="printable-content bg-white text-black p-8 font-serif leading-relaxed text-sm max-h-[70vh] overflow-y-auto">
                ...
                <div className="space-y-4 mt-6">
                    ...
                    <div><h4 className="font-bold text-center my-3">PASAL 6: PEMBATALAN</h4><p>Jika terjadi pembatalan oleh PIHAK KEDUA, maka Uang Muka (DP) yang telah dibayarkan tidak dapat dikembalikan. Ketentuan lebih lanjut diatur dalam Syarat & Ketentuan Umum.</p></div>
