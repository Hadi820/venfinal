...
interface SocialPlannerProps {
    posts: SocialMediaPost[];
    socialMediaPostCrud: CrudProps<SocialMediaPost>;
    projects: Project[];
    showNotification: (message: string) => void;
}

const SocialPlanner: React.FC<SocialPlannerProps> = ({ posts, socialMediaPostCrud, projects, showNotification }) => {
    ...
    const handleDrop = async (e: React.DragEvent<HTMLDivElement>, newStatus: PostStatus) => {
        ...
        await socialMediaPostCrud.update(postId, { status: newStatus });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (modalMode === 'add') {
            await socialMediaPostCrud.add(postData);
            showNotification('Postingan baru berhasil ditambahkan ke draf.');
        } else if (selectedPost) {
            await socialMediaPostCrud.update(selectedPost.id, postData);
    ...
    const handleDelete = async (postId: string) => {
        if(window.confirm('Yakin ingin menghapus postingan ini?')) {
            await socialMediaPostCrud.remove(postId);
