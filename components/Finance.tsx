...
const Finance: React.FC<FinanceProps> = ({ transactions, transactionCrud, pockets, pocketCrud, projects, profile, cards, cardCrud, teamMembers, rewardLedgerEntries }) => {
    ...
    const handleCloseBudget = async (budgetPocket: FinancialPocket, isAutomatic: boolean = false) => {
        ...
        if (budgetPocket.amount > 0) {
            ...
            const newSavedPocket = await pocketCrud.add(newSavedPocketData);
            if (!newSavedPocket) return;
            ...
            const closingTxData = {
                ...
                method: 'Sistem' as const,
                pocketId: budgetPocket.id
            };
            await transactionCrud.add(closingTxData);
    
            await pocketCrud.update(budgetPocket.id, { amount: 0, name: currentMonthName });
            ...
            await pocketCrud.update(budgetPocket.id, { name: currentMonthName });
            if (isAutomatic) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (type === 'transaction') {
            const { id, ...newTxData } = { ...form, amount: Number(form.amount) };
            if (mode === 'add') {
                 if (newTxData.type === TransactionType.EXPENSE) {
                    ...
                    if (source.startsWith('pocket-')) {
                        ...
                        if (!pocket || pocket.amount < newTxData.amount) { alert(`Saldo di kantong ${pocket?.name} tidak mencukupi.`); return; }
                        
                        // Deduct from source card if linked
                        if (pocket.sourceCardId) {
                            const sourceCard = cards.find(c => c.id === pocket.sourceCardId);
                            if (!sourceCard || sourceCard.balance < newTxData.amount) { alert(`Saldo di kartu sumber (${sourceCard?.bankName}) tidak mencukupi.`); return; }
                            await cardCrud.update(pocket.sourceCardId, { balance: sourceCard.balance - newTxData.amount });
                        }
                        
                        await pocketCrud.update(pocketId, { amount: pocket.amount - newTxData.amount });
                        ...
                        if (!card || card.balance < newTxData.amount) { alert(`Saldo di ${card?.bankName || 'sumber'} tidak mencukupi.`); return; }
                        await cardCrud.update(cardId, { balance: card.balance - newTxData.amount });
                ...
                await transactionCrud.add(newTxData);
            } else { // Edit mode
                 await transactionCrud.update(data.id, newTxData);
        ...
        if (type === 'card') {
            if (mode === 'add') {
                ...
                const newCard = await cardCrud.add(newCardData);
                if (newCard && initialBalance > 0) {
                    const initialTxData = {
                        date: new Date().toISOString().split('T')[0],
                    ...
                    await transactionCrud.add(initialTxData);
                }
                showNotification(`Akun baru "${form.cardHolderName}" ditambahkan.`);
            } else { // Edit mode
                const adjustmentAmount = Number(form.adjustmentAmount) || 0;
                const { balance, id, ...cardDetails } = form;
                await cardCrud.update(data.id, cardDetails);
                ...
                if (adjustmentAmount !== 0) {
                    const adjTxData = {
                        ...
                        category: (adjustmentAmount > 0 ? 'Modal' : 'Operasional Kantor') as any,
                        method: 'Sistem' as const,
                        cardId: data.id,
                    };
                    await transactionCrud.add(adjTxData);
                    const currentCard = cards.find(c => c.id === data.id);
                    if(currentCard) {
                        await cardCrud.update(data.id, { balance: currentCard.balance + adjustmentAmount });
                    }
        ...
        if (type === 'pocket') {
            const { id, ...pocketData } = form;
            if (mode === 'add') {
                await pocketCrud.add({ ...pocketData, amount: 0 });
            } else {
                await pocketCrud.update(data.id, pocketData);
            }
        }
        if (type === 'transfer') {
            ...
            if (isDeposit) {
                ...
                await cardCrud.update(card.id, { balance: card.balance - amount });
                await pocketCrud.update(pocket.id, { amount: pocket.amount + amount, sourceCardId: card.id });
            } else { // Withdraw
                if (pocket.amount < amount) {
                    alert(`Saldo kantong ${pocket.name} tidak mencukupi.`);
                    return;
                }
                await pocketCrud.update(pocket.id, { amount: pocket.amount - amount });
                await cardCrud.update(card.id, { balance: card.balance + amount });
            }
            ...
            const transferTxData = {
                ...
                type: TransactionType.EXPENSE,
                category: 'Transfer Internal' as const,
                method: 'Sistem' as const,
                cardId: card.id,
                pocketId: pocket.id,
            };
            await transactionCrud.add(transferTxData);
        }
         if (type === 'topup-cash') {
            const amount = Number(form.amount);
            ...
            if (!sourceCard || !cashCard) { alert("Kartu sumber atau kas tidak valid."); return; }
            if (sourceCard.balance < amount) { alert(`Saldo kartu ${sourceCard.bankName} tidak mencukupi.`); return; }
            
            await cardCrud.update(sourceCard.id, { balance: sourceCard.balance - amount });
            await cardCrud.update(cashCard.id, { balance: cashCard.balance + amount });

            const topupTxData = {
                date: new Date().toISOString().split('T')[0],
                description: `Top-up saldo tunai dari ${sourceCard.bankName}`, amount,
                type: TransactionType.EXPENSE, category: 'Transfer Internal' as const, method: 'Sistem' as const, cardId: sourceCard.id
            };
            await transactionCrud.add(topupTxData);
        }
        handleCloseModal();
    };
    ...
    const handleDelete = async (type: 'transaction' | 'pocket' | 'card', id: string) => {
        ...
        if (type === 'transaction') await transactionCrud.remove(id);
        if (type === 'pocket') await pocketCrud.remove(id);
        if (type === 'card') await cardCrud.remove(id);
    };
