            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={modalMode === 'add' ? 'Tambah Status Baru' : `Edit Status: ${selectedStatus?.name}`}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="input-group md:col-span-2"><input type="text" id="name" name="name" value={form.name} onChange={handleFormChange} className="input-field" required placeholder=" " aria-describedby="statusNameHelp"/><label htmlFor="name" className="input-label">Nama Status</label><p id="statusNameHelp" className="input-help">Contoh: Dalam Proses, Siap Kirim, Selesai.</p></div>
                        <div className="input-group"><input type="color" id="color" name="color" value={form.color} onChange={handleFormChange} className="input-field !p-1 h-12" aria-describedby="colorHelp"/><label htmlFor="color" className="input-label">Warna</label><p id="colorHelp" className="input-help">Warna label untuk menandai status di tampilan proyek.</p></div>
                    </div>
...
interface SettingsProps {
    ...
    profileCrud: {
        create: (profileData: Omit<Profile, 'id' | 'user_id'>) => Promise<Profile | undefined>;
        update: (updates: Partial<Profile>) => Promise<Profile | undefined>;
    };
    transactions: Transaction[];
    projects: Project[];
    users: User[];
    userCrud: CrudProps<User>;
...
const Settings: React.FC<SettingsProps> = ({ profile, setProfile, profileCrud, transactions, projects, users, userCrud, currentUser }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        await profileCrud.update(localProfile);
        setShowSuccess(true);
    ...
    const handleUserFormSubmit = async (e: React.FormEvent) => {
        ...
        if (userModalMode === 'add') {
            ...
            if (userForm.password) {
                updatedUserData.password = userForm.password;
            }
            await userCrud.update(selectedUser.id, updatedUserData);
        }
    ...
    const handleCategoryUpdate = async (field: keyof Profile, updatedCategories: string[]) => {
        await profileCrud.update({ [field]: updatedCategories });
    };
    ...
    const handleAddOrUpdateIncomeCategory = () => {
        ...
        const categories = localProfile.incomeCategories || [];
        if (editingIncomeCategory) {
            if (newCategory !== editingIncomeCategory && categories.includes(newCategory)) { alert('Kategori ini sudah ada.'); return; }
            handleCategoryUpdate('incomeCategories', categories.map(c => c === editingIncomeCategory ? newCategory : c).sort());
            setEditingIncomeCategory(null);
        } else {
            if (categories.includes(newCategory)) { alert('Kategori ini sudah ada.'); return; }
            handleCategoryUpdate('incomeCategories', [...categories, newCategory].sort());
        }
    ...
    const handleDeleteIncomeCategory = (category: string) => {
        ...
        if (isCategoryInUse) { alert(`Kategori "${category}" tidak dapat dihapus karena sedang digunakan dalam transaksi.`); return; }
        if (window.confirm(`Yakin ingin menghapus kategori "${category}"?`)) {
            handleCategoryUpdate('incomeCategories', (localProfile.incomeCategories || []).filter(c => c !== category));
        }
    };
    ...
    const handleAddOrUpdateExpenseCategory = () => {
        ...
        const categories = localProfile.expenseCategories || [];
        if (editingExpenseCategory) {
            if (newCategory !== editingExpenseCategory && categories.includes(newCategory)) { alert('Kategori ini sudah ada.'); return; }
            handleCategoryUpdate('expenseCategories', categories.map(c => c === editingExpenseCategory ? newCategory : c).sort());
            setEditingExpenseCategory(null);
        } else {
            if (categories.includes(newCategory)) { alert('Kategori ini sudah ada.'); return; }
            handleCategoryUpdate('expenseCategories', [...categories, newCategory].sort());
        }
    ...
    const handleDeleteExpenseCategory = (category: string) => {
        ...
        if (isCategoryInUse) { alert(`Kategori "${category}" tidak dapat dihapus karena sedang digunakan dalam transaksi.`); return; }
        if (window.confirm(`Yakin ingin menghapus kategori "${category}"?`)) {
            handleCategoryUpdate('expenseCategories', (localProfile.expenseCategories || []).filter(c => c !== category));
        }
    };
    ...
    const handleAddOrUpdateProjectType = () => {
        ...
        const types = localProfile.projectTypes || [];
        if (editingProjectType) {
            if (newType !== editingProjectType && types.includes(newType)) { alert('Jenis proyek ini sudah ada.'); return; }
            handleCategoryUpdate('projectTypes', types.map(t => t === editingProjectType ? newType : t).sort());
            setEditingProjectType(null);
        } else {
            if (types.includes(newType)) { alert('Jenis proyek ini sudah ada.'); return; }
            handleCategoryUpdate('projectTypes', [...types, newType].sort());
        }
        setProjectTypeInput('');
    };
    ...
    const handleDeleteProjectType = (type: string) => {
        ...
        if (isTypeInUse) { alert(`Jenis proyek "${type}" tidak dapat dihapus karena sedang digunakan.`); return; }
        if (window.confirm(`Yakin ingin menghapus jenis proyek "${type}"?`)) {
            handleCategoryUpdate('projectTypes', (localProfile.projectTypes || []).filter(t => t !== type));
        }
    };
    ...
    const handleAddOrUpdateEventType = () => {
        ...
        const types = localProfile.eventTypes || [];
        if (editingEventType) {
            if (newType !== editingEventType && types.includes(newType)) { alert('Jenis acara ini sudah ada.'); return; }
            handleCategoryUpdate('eventTypes', types.map(t => t === editingEventType ? newType : t).sort());
            setEditingEventType(null);
        } else {
            if (types.includes(newType)) { alert('Jenis acara ini sudah ada.'); return; }
            handleCategoryUpdate('eventTypes', [...types, newType].sort());
        }
        setEventTypeInput('');
    };
    ...
    const handleDeleteEventType = (type: string) => {
        ...
        if (window.confirm(`Yakin ingin menghapus jenis acara "${type}"?`)) {
            handleCategoryUpdate('eventTypes', (localProfile.eventTypes || []).filter(t => t !== type));
        }
    };
    ...
    const handleDeleteSopCategory = (cat: string) => {
        if (window.confirm(`Yakin ingin menghapus kategori SOP "${cat}"?`)) {
            handleCategoryUpdate('sopCategories', (localProfile.sopCategories || []).filter(c => c !== cat));
        }
    };
                        ...
                        <div className="space-y-6">
                            <h3 className="text-lg font-semibold text-brand-text-light border-b border-gray-700/50 pb-3">Informasi Publik</h3>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                                                        <div className="input-group"><input id="fullName" type="text" name="fullName" value={localProfile.fullName} onChange={handleInputChange} className="input-field" placeholder=" " /><label htmlFor="fullName" className="input-label">Nama Lengkap</label></div>
                                                        <div className="input-group"><input id="companyName" type="text" name="companyName" value={localProfile.companyName} onChange={handleInputChange} className="input-field" placeholder=" " /><label htmlFor="companyName" className="input-label">Nama Perusahaan</label></div>
                                                        <div className="input-group"><input id="email" type="email" name="email" value={localProfile.email} onChange={handleInputChange} className="input-field" placeholder=" " /><label htmlFor="email" className="input-label">Email</label></div>
                                                        <div className="input-group"><input id="phone" type="tel" name="phone" value={localProfile.phone} onChange={handleInputChange} className="input-field" placeholder=" " /><label htmlFor="phone" className="input-label">No. WhatsApp</label></div>
                                                        <div className="input-group"><input id="website" type="url" name="website" value={localProfile.website} onChange={handleInputChange} className="input-field" placeholder=" " /><label htmlFor="website" className="input-label">Website</label></div>
                                                    </div>
                                                    <div className="input-group"><input id="address" type="text" name="address" value={localProfile.address} onChange={handleInputChange} className="input-field" placeholder=" " /><label htmlFor="address" className="input-label">Alamat</label></div>
                                                    <div className="input-group"><input id="bankAccount" type="text" name="bankAccount" value={localProfile.bankAccount} onChange={handleInputChange} className="input-field" placeholder=" " aria-describedby="bankHelp"/><label htmlFor="bankAccount" className="input-label">Rekening Bank</label><p id="bankHelp" className="input-help">Format: Nama Bank - Nomor Rekening - Nama Pemilik.</p></div>
                                                    <div className="input-group"><input id="authorizedSigner" type="text" name="authorizedSigner" value={localProfile.authorizedSigner} onChange={handleInputChange} className="input-field" placeholder=" " /><label htmlFor="authorizedSigner" className="input-label">Nama Penanggung Jawab Tanda Tangan (u/ QR Code)</label></div>
                                                    <div className="input-group"><textarea id="bio" name="bio" value={localProfile.bio} onChange={handleInputChange} className="input-field" placeholder=" " rows={3}></textarea><label htmlFor="bio" className="input-label">Bio Perusahaan</label></div>
                                                    <div className="input-group"><textarea id="briefingTemplate" name="briefingTemplate" value={localProfile.briefingTemplate} onChange={handleInputChange} className="input-field" placeholder=" " rows={3}></textarea><label htmlFor="briefingTemplate" className="input-label">Template Pesan Briefing Tim</label></div>
                                                    <div className="input-group"><textarea id="termsAndConditions" name="termsAndConditions" value={localProfile.termsAndConditions || ''} onChange={handleInputChange} className="input-field" placeholder=" " rows={15}></textarea><label htmlFor="termsAndConditions" className="input-label">Syarat & Ketentuan</label></div>
                            </div>

                            <h3 className="text-lg font-semibold text-brand-text-light border-b border-gray-700/50 pb-3 mt-8">Notifikasi</h3>
                            <div className="space-y-4 max-w-2xl mx-auto">
                                <div className="flex justify-between items-center"><label htmlFor="notif-newProject">Proyek Baru Dibuat</label><ToggleSwitch id="notif-newProject" enabled={localProfile.notificationSettings.newProject} onChange={() => handleNotificationChange('newProject')} /></div>
                                <div className="flex justify-between items-center"><label htmlFor="notif-paymentConfirmation">Konfirmasi Pembayaran Diterima</label><ToggleSwitch id="notif-paymentConfirmation" enabled={localProfile.notificationSettings.paymentConfirmation} onChange={() => handleNotificationChange('paymentConfirmation')} /></div>
                                <div className="flex justify-between items-center"><label htmlFor="notif-deadlineReminder">Pengingat Deadline Proyek</label><ToggleSwitch id="notif-deadlineReminder" enabled={localProfile.notificationSettings.deadlineReminder} onChange={() => handleNotificationChange('deadlineReminder')} /></div>
                            </div>

                            <h3 className="text-lg font-semibold text-brand-text-light border-b border-gray-700/50 pb-3 mt-8">Keamanan</h3>
                            <div className="space-y-4 max-w-2xl mx-auto">
                                <div className="flex justify-between items-center"><label htmlFor="security-2fa">Autentikasi Dua Faktor (2FA)</label><ToggleSwitch id="security-2fa" enabled={localProfile.securitySettings.twoFactorEnabled} onChange={() => setLocalProfile(p => ({ ...p, securitySettings: { ...p.securitySettings, twoFactorEnabled: !p.securitySettings.twoFactorEnabled } }))} /></div>
                            </div>
