...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, isOpen, setIsOpen, handleLogout, currentUser }) => {
  ...
  <aside id="sidebar" className={`sidebar fixed xl:relative inset-y-0 left-0 w-64 bg-brand-surface flex-col flex-shrink-0 flex z-40 transform transition-transform duration-300 ease-in-out xl:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
