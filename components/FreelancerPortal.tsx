...
const PaymentsTab: React.FC<{freelancer: TeamMember, projects: Project[], teamProjectPayments: TeamProjectPayment[], teamPaymentRecords: TeamPaymentRecord[], onSlipView: (record: TeamPaymentRecord) => void}> = ({ freelancer, projects, teamProjectPayments, teamPaymentRecords, onSlipView }) => {
    const payments = teamProjectPayments.filter(p => p.teamMemberId === freelancer.id);
    const totalPaid = payments.filter(p => p.status === 'Paid').reduce((s, p) => s + (p.fee || 0), 0);
    const totalUnpaid = payments.filter(p => p.status !== 'Paid').reduce((s, p) => s + (p.fee || 0), 0);
    ...
    <div className="bg-brand-surface p-6 rounded-2xl shadow-lg border border-brand-border widget-animate">
        ...
        <div className="mb-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-3 bg-white/5 rounded-lg">
                <p className="text-xs text-brand-text-secondary">Total Diterima</p>
                <p className="text-lg font-semibold text-green-400">{formatCurrency(totalPaid)}</p>
            </div>
            <div className="p-3 bg-white/5 rounded-lg">
                <p className="text-xs text-brand-text-secondary">Total Belum Dibayar</p>
                <p className="text-lg font-semibold text-yellow-400">{formatCurrency(totalUnpaid)}</p>
            </div>
        </div>
        ...
        <div className="overflow-x-auto"><table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-brand-border">
                {payments.map((p, index) => {
                    ...
                    const paymentRecord = isPaid ? teamPaymentRecords.find(rec => rec.projectPaymentIds.includes(p.id) || (rec.projectPaymentIds || []).includes(p.id)) : null;
                    return (
                        <tr key={p.id} className="widget-animate" style={{ animationDelay: `${index * 50}ms`}}>
                            ...
                            <td className="p-3 text-right font-medium text-brand-text-primary">{formatCurrency(p.fee || 0)}</td>
                            <td className="p-3 text-center space-y-1">
                                <div className="flex items-center justify-center gap-2">
                                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${isPaid ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>{isPaid ? 'Lunas' : 'Belum Lunas'}</span>
                                    {paymentRecord && (
                                        <button onClick={() => onSlipView(paymentRecord)} className="text-xs font-semibold text-brand-accent hover:underline">Lihat Kwitansi</button>
                                    )}
                                </div>
                                {isPaid && !paymentRecord && <div className="text-xs text-brand-text-secondary">Lunas (Slip belum dibuat)</div>}
                            </td>
                        </tr>
                    )
                })}
                {payments.length === 0 && (
                    <tr><td colSpan={4} className="p-6 text-center text-brand-text-secondary">Belum ada riwayat pembayaran.</td></tr>
                )}
            </tbody>
        </table></div>
    </div>
...
const PerformanceTab: React.FC<{freelancer: TeamMember}> = ({ freelancer }) => (
    <div className="space-y-6">
        ...
        <div className="flex justify-center items-center gap-2"><StarIcon className="w-8 h-8 text-yellow-400 fill-current" />
            <p className="text-3xl font-bold text-brand-text-light">{(typeof freelancer.rating === 'number' && !isNaN(freelancer.rating)) ? freelancer.rating.toFixed(1) : 'N/A'} / 5.0</p>
        </div>
        </div>
