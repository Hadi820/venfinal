// ...existing code...
// Tambahkan deklarasi state projects dan setProjects jika belum ada
// Jika sudah ada, pastikan setProjects dideklarasikan dengan benar
...
const ProjectListView: React.FC<ProjectListViewProps> = ({ projects, handleOpenDetailModal, handleOpenForm, handleProjectDelete, config }) => {
    ...
    const ProgressBar: React.FC<{ progress: number, status: string, config: ProjectStatusConfig[] }> = ({ progress, status, config }) => (
        <div className="w-full bg-gray-700 rounded-full h-1.5">
            <div className="h-1.5 rounded-full transition-all duration-300" style={{ width: `${progress}%`, backgroundColor: getStatusColor(status, config) || '#64748b' }}></div>
        </div>
    );
...
const ProjectDetailModal: React.FC<ProjectDetailModalProps> = ({ selectedProject, setSelectedProject, teamMembers, clients, profile, showNotification, projectCrud, onClose, handleOpenForm, handleProjectDelete, handleOpenBriefingModal, handleOpenConfirmationModal, packages }) => {
    ...
    const handleAddRevision = async (e: React.FormEvent) => {
        ...
        const updatedProject = await projectCrud.update(selectedProject.id, { revisions: updatedRevisions });
    ...
    const handleToggleDigitalItem = async (itemText: string) => {
        ...
        const updatedProject = await projectCrud.update(selectedProject.id, { completedDigitalItems: newCompleted });
        if (updatedProject) {
...
interface ProjectsProps {
    ...
    teamProjectPaymentCrud: CrudProps<TeamProjectPayment>;
    transactions: Transaction[];
    transactionCrud: CrudProps<Transaction>;
    initialAction: NavigationAction | null;
    setInitialAction: (action: NavigationAction | null) => void;
    profile: Profile;
    showNotification: (message: string) => void;
    cards: Card[];
    cardCrud: CrudProps<Card>;
}
...
const ConfirmationModal: React.FC<{
    ...
    projectCrud: CrudProps<Project>;
    onClose: () => void;
}> = ({ project, subStatus, isFollowUp, clients, teamMembers, projectCrud, onClose }) => {
...
*Tim Vena Pictures*`;
    ...
    const handleShare = async () => {
        ...
        if (phoneNumber) {
            ...
            await projectCrud.update(project.id, { subStatusConfirmationSentAt: newConfirmationSentAt });
...
export const Projects: React.FC<ProjectsProps> = ({ projects, projectCrud, clients, packages, teamMembers, teamProjectPayments, teamProjectPaymentCrud, transactions, transactionCrud, initialAction, setInitialAction, profile, showNotification, cards, cardCrud }) => {
    // State untuk manipulasi projects
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        if (formMode === 'add') {
            const newProjectData = {
                ...
                packageId: '', // Will be set on client page
                addOns: [], // Will be set on client page
            };
            const newProject = await projectCrud.add(newProjectData);
            if (newProject) {
                await updateTeamPayments(newProject);
            }
        } else { // edit mode
            const originalProject = projects.find(p => p.id === id);
            if (!originalProject) return;

            // Ensure progress follows status if status was changed via the form
            const newStatus = (formDataWithoutId as any).status || originalProject.status;
            const newProgress = getProgressForStatus(newStatus, profile.projectStatusConfig);

            const payloadToUpdate = { ...formDataWithoutId, progress: newProgress };

            const updatedProject = await projectCrud.update(id, payloadToUpdate);
            if (!updatedProject) return;

            await updateTeamPayments(updatedProject);

            if (updatedProject.status === 'Cetak') {
                await handlePrintingCosts(originalProject, updatedProject);
            }
            await handleOtherCosts(originalProject, updatedProject);
        }

        handleCloseForm();
    };

    const handlePrintingCosts = async (originalProject: Project, updatedProject: Project) => {
        const originalItems = originalProject.printingDetails || [];
        const newItems = updatedProject.printingDetails || [];
        ...
        const itemsToDelete = originalItems.filter(oldItem => !newItems.some(newItem => newItem.id === oldItem.id));
        const itemsToAdd = newItems.filter(newItem => !originalItems.some(oldItem => oldItem.id === newItem.id));
        const itemsToUpdate = newItems.filter(newItem => {
            const oldItem = originalItems.find(item => item.id === newItem.id);
            return oldItem && oldItem.cost !== newItem.cost;
        });

        for (const item of itemsToDelete) {
            const tx = transactions.find(t => t.printingItemId === item.id);
            if (tx) await transactionCrud.remove(tx.id);
        }
        ...
        for (const item of itemsToUpdate) {
            const tx = transactions.find(t => t.printingItemId === item.id);
            if (tx) await transactionCrud.update(tx.id, { amount: item.cost });
        }
    };

    const handleOtherCosts = async (originalProject: Project, updatedProject: Project) => {
        ...
        if (originalProject.printingCost !== updatedProject.printingCost) fieldsToProcess.push('printingCost');
        if (originalProject.transportCost !== updatedProject.transportCost) fieldsToProcess.push('transportCost');

        for (const field of fieldsToProcess) {
            const cost = updatedProject[field] || 0;
            const category = field === 'printingCost' ? 'Cetak Album' : 'Transportasi';
            const description = field === 'printingCost' ? `Biaya Cetak - ${updatedProject.projectName}` : `Biaya Transportasi - ${updatedProject.projectName}`;
            const txId = `TRN-COST-${field.replace('Cost','')}-${updatedProject.id}`;

            const existingTx = transactions.find(t => t.id === txId);

            if (existingTx) {
                if (cost > 0) {
                    await transactionCrud.update(txId, { amount: cost });
                } else {
                    await transactionCrud.remove(txId);
                }
            } else if (cost > 0) {
                const newTx = {
    ...
    const updateTeamPayments = async (projectData: Project) => {
        const existingPayments = teamProjectPayments.filter(p => p.projectId === projectData.id);
        const newTeamIds = new Set(projectData.team.map(t => t.memberId));
        
        // Delete payments for members no longer on the team
        for (const payment of existingPayments) {
            if (!newTeamIds.has(payment.teamMemberId)) {
                await teamProjectPaymentCrud.remove(payment.id);
            }
        }

        // Add or update payments for current team members
        for (const teamMember of projectData.team) {
            const existingPayment = existingPayments.find(p => p.teamMemberId === teamMember.memberId);
            if (existingPayment) {
                // Update if fee or reward changed
                if (existingPayment.fee !== teamMember.fee || existingPayment.reward !== teamMember.reward) {
                    await teamProjectPaymentCrud.update(existingPayment.id, { fee: teamMember.fee, reward: teamMember.reward || 0 });
                }
            } else {
                ...
                await teamProjectPaymentCrud.add(newPaymentEntry);
            }
        }
    };
    ...
    const handleProjectDelete = async (projectId: string) => {
        if (window.confirm("Apakah Anda yakin ingin menghapus proyek ini? Semua data terkait (termasuk tugas tim dan transaksi) akan dihapus.")) {
            const paymentsToDelete = teamProjectPayments.filter(p => p.projectId === projectId);
            const transactionsToDelete = transactions.filter(t => t.projectId === projectId);

            await Promise.all([
                ...paymentsToDelete.map(p => teamProjectPaymentCrud.remove(p.id)),
                ...transactionsToDelete.map(t => transactionCrud.remove(t.id)),
                projectCrud.remove(projectId)
            ]);
    ...
    const handleDrop = async (e: React.DragEvent<HTMLDivElement>, newStatus: string) => {
        ...
        if (projectToUpdate && projectToUpdate.status !== newStatus) {
            await projectCrud.update(projectId, {
                status: newStatus,
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-brand-surface p-4 rounded-xl shadow-lg border border-brand-border flex flex-col md:flex-row justify-between items-center gap-4">
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto">
                            <select value={statusFilter || 'all'} onChange={e => setStatusFilter(e.target.value)} className="input-field !rounded-lg !border !bg-brand-bg p-2.5 w-full md:w-48">
