...
const formatRating = (r: number | null | undefined, decimals = 1) => {
    return (typeof r === 'number' && Number.isFinite(r)) ? r.toFixed(decimals) : 'N/A';
}
...
interface FreelancersProps {
    ...
    rewardLedgerEntryCrud: CrudProps<RewardLedgerEntry>;
    pockets: FinancialPocket[];
    pocketCrud: CrudProps<FinancialPocket>;
    cards: Card[];
...
export const Freelancers: React.FC<FreelancersProps> = ({
    teamMembers, teamMemberCrud, teamProjectPayments, teamProjectPaymentCrud, teamPaymentRecords, teamPaymentRecordCrud,
    transactions, transactionCrud, userProfile, showNotification, initialAction, setInitialAction, projects, projectCrud,
    rewardLedgerEntries, rewardLedgerEntryCrud, pockets, pocketCrud, cards, cardCrud, onSignPaymentRecord
}) => {
    ...
    // No longer generate QR images in-browser; we show the direct URL instead so
    // it can be opened on phones. Keep state only for modal content.
    useEffect(() => {
        // noop - preserved to keep dependency shape if other hooks rely on qrModalContent
    }, [qrModalContent]);

    const handleOpenQrModal = (member: TeamMember) => {
    // Build an absolute URL at runtime so copied/opened links include the current origin (e.g., http://localhost:5173)
    const url = `${window.location.origin}${window.location.pathname}#/freelancer-portal/${member.portalAccessId}`;
    setQrModalContent({ title: `Portal untuk ${member.name}`, url });
    };
    ...
    const teamStats = useMemo(() => {
        ...
        return {
            ...
            topRatedRating: topRated ? formatRating(topRated.rating) : 'N/A',
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formMode === 'add') {
            ...
            // Ensure we don't unintentionally overwrite rating here - rating is managed separately
            const updates = { ...formData } as any;
            if ('rating' in updates) delete updates.rating;
            await teamMemberCrud.update(selectedMember.id, updates);
    ...
    const handleDelete = async (memberId: string) => {
        ...
        if (window.confirm("Apakah Anda yakin ingin menghapus freelancer ini?")) {
            await teamMemberCrud.remove(memberId);
        }
    };
    ...
    const handlePay = async () => {
        ...
        if (paymentSourceId.startsWith('card-')) {
            ...
            await cardCrud.update(cardId, { balance: card.balance - paymentAmount });
            (newTransactionData as Partial<Transaction>).cardId = cardId;
        } else { // pocket
            ...
            await pocketCrud.update(pocketId, { amount: pocket.amount - paymentAmount });
            (newTransactionData as Partial<Transaction>).pocketId = pocketId;
            (newTransactionData as Partial<Transaction>).method = 'Sistem';
        }
        ...
        await teamPaymentRecordCrud.add(newRecordData);

        // 4. Update Project Payment Status
        await Promise.all(
            projectsToPay.map(id => teamProjectPaymentCrud.update(id, { status: 'Paid' }))
        );
    ...
    const handleWithdrawRewards = async () => {
        ...
        if (window.confirm(`Anda akan menarik saldo hadiah sebesar ${formatCurrency(selectedMember.rewardBalance)} untuk ${selectedMember.name}. Lanjutkan?`)) {
            ...
            const ledgerEntryData = {
                teamMemberId: selectedMember.id,
                date: withdrawalTxData.date,
                description: withdrawalTxData.description,
                amount: -withdrawalAmount,
            };

            // 3. Update states
            await transactionCrud.add(withdrawalTxData);
            await rewardLedgerEntryCrud.add(ledgerEntryData);
            await cardCrud.update(sourceCard.id, { balance: sourceCard.balance - withdrawalAmount });
            await teamMemberCrud.update(selectedMember.id, { rewardBalance: 0 });
            
            const rewardPocket = pockets.find(p => p.type === PocketType.REWARD_POOL);
            if (rewardPocket) {
                await pocketCrud.update(rewardPocket.id, { amount: rewardPocket.amount - withdrawalAmount });
            }
    ...
    const handleSetRating = async (rating: number) => {
        if (!selectedMember) return;
        const updatedMember = await teamMemberCrud.update(selectedMember.id, { rating });
        if (updatedMember) {
    ...
    const handleAddNote = async () => {
        ...
        const updatedMember = await teamMemberCrud.update(selectedMember.id, { performanceNotes: updatedNotes });
        if(updatedMember) {
            setSelectedMember(updatedMember);
            setNewNote('');
                        ...
                        <tbody className="divide-y divide-brand-border">
                            {teamMembers.map(member => {
                                ...
                                <tr key={member.id} className="hover:bg-brand-bg">
                                    ...
                                    <td className="px-4 py-3"><div className="flex justify-center items-center gap-1"><StarIcon className="w-4 h-4 text-yellow-400 fill-current"/>{formatRating(member.rating)}</div></td>
             ...
             {qrModalContent && (
                 <Modal isOpen={!!qrModalContent} onClose={() => setQrModalContent(null)} title={qrModalContent.title} size="sm">
                    <div className="text-center p-4">
                        <p className="text-sm text-brand-text-secondary mb-3">Tautan portal (bisa dibuka di HP):</p>
                        <a href={qrModalContent.url} target="_blank" rel="noopener noreferrer" className="break-all text-blue-500 hover:underline">{qrModalContent.url}</a>
                        <div className="flex gap-2 mt-4">
                            <button onClick={() => { navigator.clipboard.writeText(qrModalContent.url); }} className="button-secondary w-full">Salin Tautan</button>
                            <button onClick={() => { window.open(qrModalContent.url, '_blank'); }} className="button-primary w-full">Buka di Tab Baru</button>
                        </div>
                    </div>
                 </Modal>
             )}
