...
export const CalendarView: React.FC<CalendarViewProps> = ({ projects, projectCrud, teamMembers, profile }) => {
    // STATE
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        if (selectedEvent) { // Editing
            const updatedEventData = {
                projectName: eventForm.projectName, projectType: eventForm.projectType,
            ...
            const updatedEvent = await projectCrud.update(selectedEvent.id, updatedEventData);
            if (updatedEvent) {
                setSelectedEvent(updatedEvent);
            }
            setPanelMode('detail');
        } else { // Adding
            const dateToUse = eventForm.date && eventForm.date !== '' ? eventForm.date : new Date().toISOString().split('T')[0];

            const newEventData = {
                ...eventForm,
                date: dateToUse,
                // clientId must be a UUID or null — avoid sending literal strings like 'INTERNAL' which break Postgres UUID type
            ...
            await projectCrud.add(newEventData);
            setIsPanelOpen(false);
        }
    };
    ...
    const handleDeleteEvent = async () => {
        if (selectedEvent && window.confirm(`Yakin ingin menghapus acara "${selectedEvent.projectName}"?`)) {
            await projectCrud.remove(selectedEvent.id);
            setIsPanelOpen(false);
        }
    };
