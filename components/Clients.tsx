...
interface ClientsProps {
    ...
    promoCodes: PromoCode[];
    promoCodeCrud: CrudProps<PromoCode>;
...
const Clients: React.FC<ClientsProps> = ({ clients, clientCrud, projects, projectCrud, packages, addOns, transactions, transactionCrud, userProfile, showNotification, initialAction, setInitialAction, cards, cardCrud, pockets, pocketCrud, contracts, handleNavigation, clientFeedback, promoCodes, promoCodeCrud, onSignInvoice, onSignTransaction }) => {
    ...
    // No in-browser QR generation anymore. We keep qrModalContent so we can show
    // the link and provide copy/open actions for phones.
    useEffect(() => {
        // noop
    ...
    const handleOpenQrModal = (client: Client) => {
        ...
        setQrModalContent({ title: `Portal untuk ${client.name}`, url });
    };
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        if (modalMode === 'add') {
             ...
             if (!selectedClient) { // New client
                ...
                const newClient = await clientCrud.add(newClientData);
                if (!newClient) return;
            ...
            if (newProject && newProject.amountPaid > 0) {
                ...
                const cardToUpdate = cards.find(c => c.id === formData.dpDestinationCardId);
                if (cardToUpdate) {
                    await cardCrud.update(cardToUpdate.id, { balance: cardToUpdate.balance + newProject.amountPaid });
                }
            }
             if (promoCode) {
                await promoCodeCrud.update(promoCode.id, { usageCount: promoCode.usageCount + 1 });
            }
            ...
            await clientCrud.update(selectedClient.id, { name: formData.clientName, email: formData.email, phone: formData.phone, whatsapp: formData.whatsapp, instagram: formData.instagram, clientType: formData.clientType });
            
            const amountPaid = selectedProject.amountPaid; // Keep existing payment data
            const remainingPayment = totalProject - amountPaid;
            await projectCrud.update(selectedProject.id, {
                projectName: formData.projectName,
                projectType: formData.projectType,
                location: formData.location,
                date: formData.date,
    ...
    const handleDeleteClient = async (clientId: string) => {
        if (window.confirm("Menghapus klien akan menghapus semua proyek dan transaksi terkait. Apakah Anda yakin?")) {
            ...
            const transactionsToDelete = transactions.filter(t => projectIdsToDelete.includes(t.projectId || ''));

            // Use Promise.all to run deletions in parallel for efficiency
            await Promise.all([
                ...transactionsToDelete.map(t => transactionCrud.remove(t.id)),
                ...projectIdsToDelete.map(id => projectCrud.remove(id)),
                clientCrud.remove(clientId)
            ]);
    ...
    const handleRecordPayment = async (projectId: string, amount: number, destinationCardId: string) => {
        ...
        const cardToUpdate = cards.find(c => c.id === destinationCardId);
        if (cardToUpdate) {
            await cardCrud.update(destinationCardId, { balance: cardToUpdate.balance + amount });
        }
    
        const newAmountPaid = project.amountPaid + amount;
        const remaining = project.totalCost - newAmountPaid;
        await projectCrud.update(project.id, {
        ...
        <div className="space-y-6">
            ...
            {isBookingFormShareModalOpen && (
                <Modal isOpen={isBookingFormShareModalOpen} onClose={() => setIsBookingFormShareModalOpen(false)} title="Bagikan Formulir Booking Publik" size="sm">
                    <div className="text-center p-4">
                        <p className="text-sm text-brand-text-secondary mb-3">Tautan formulir booking publik:</p>
                        <a href={bookingFormUrl} target="_blank" rel="noopener noreferrer" className="break-all text-blue-500 hover:underline">{bookingFormUrl}</a>
                        <div className="flex items-center gap-2 mt-6">
                            <button onClick={copyBookingLinkToClipboard} className="button-secondary w-full">Salin Tautan</button>
                            <button onClick={() => { window.open(bookingFormUrl, '_blank'); setIsBookingFormShareModalOpen(false); }} className="button-primary w-full">Buka di Tab Baru</button>
                        </div>
                    </div>
                </Modal>
            )}
