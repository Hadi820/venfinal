...
interface ContractsProps {
    contracts: Contract[];
    contractCrud: CrudProps<Contract>;
    clients: Client[];
    projects: Project[];
    profile: Profile;
...
const Contracts: React.FC<ContractsProps> = ({ contracts, contractCrud, clients, projects, profile, showNotification, initialAction, setInitialAction, packages, onSignContract }) => {
    ...
    const [portalBaseOverride, setPortalBaseOverride] = useState<string | null>(() => {
        try { return localStorage.getItem('VENA_PORTAL_BASE') || null; } catch (e) { return null; }
    });
    ...
    // Deprecated: in-browser QR generation removed. We keep state to render a link modal.
    useEffect(() => {
        // noop
    }, [qrModalContent]);

    const getEffectivePortalBase = () => {
        const configuredBase = import.meta.env.VITE_PUBLIC_URL as string | undefined;
        const override = portalBaseOverride && portalBaseOverride.length > 0 ? portalBaseOverride : undefined;
        const runtimeBase = profile?.website && profile.website.length > 0 ? profile.website : undefined;
        const chosenBase = configuredBase && configuredBase.length > 0 ? configuredBase : (override ? override : (runtimeBase && runtimeBase.length > 0 ? runtimeBase : window.location.origin));
        return (chosenBase || window.location.origin).replace(/\/$/, '');
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (modalMode === 'add') {
            ...
            await contractCrud.add(newContractData);
            ...
            await contractCrud.update(selectedContract.id, updatedContractData);
    ...
    const handleDelete = async (contractId: string) => {
        if (window.confirm("Apakah Anda yakin ingin menghapus kontrak ini?")) {
            await contractCrud.remove(contractId);
