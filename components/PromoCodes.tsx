...
const PromoCodes: React.FC<PromoCodesProps> = ({ promoCodes, promoCodeCrud, projects, showNotification }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (modalMode === 'add') {
            ...
            await promoCodeCrud.add(newCodeData);
            showNotification(`Kode promo "${formData.code.toUpperCase()}" berhasil dibuat.`);
        } else if (selectedCode) {
            await promoCodeCrud.update(selectedCode.id, promoData);
            showNotification(`Kode promo "${formData.code.toUpperCase()}" berhasil diperbarui.`);
        }
        handleCloseModal();
    };
    ...
    const handleDelete = async (codeId: string) => {
        ...
        if (window.confirm("Apakah Anda yakin ingin menghapus kode promo ini?")) {
            await promoCodeCrud.remove(codeId);
