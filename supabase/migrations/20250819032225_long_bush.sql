/*
  ...
  1. New Tables
    ...
    - `packages` - Service packages with pricing and items
    - `add_ons` - Additional services that can be added to packages
    - `projects` - Main projects with status tracking and team assignments
    ...
    - `financial_pockets` - Budget categories and savings goals
    - `team_project_payments` - Payments owed to team members for projects
    - `team_payment_records` - Records of actual payments made to team members
    - `leads` - Potential clients and prospects
    - `reward_ledger_entries` - Reward system transactions for team members
...
CREATE POLICY "Users can update their own data" ON users FOR UPDATE TO authenticated USING (auth.uid()::text = id::text);
